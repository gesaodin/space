...
class MHistorialMovimiento extends CI_Model{
	...
	function listar($cedula = ''){
		$arr = array();
		$sConsulta = 'select tipo_movimiento_id, sum(monto) AS monto from movimiento where cedula =\'' . $cedula . '\' GROUP BY tipo_movimiento_id';
	...
	public function listarPorComponente($idComponente = 0){
		$sConsulta = 'SELECT 
										...
										SUM(movimiento.monto) AS monto FROM beneficiario 
	  ...
	  $obj = $this->Dbpace->consultar($sConsulta);
		...
		foreach ($obj->rs as $clv => $val) {		
			...
			$lstH[$val->tipo_movimiento_id] =$val->monto;
			$lst[$ced] = $lstH;
			$i++;				
		}
		/**
