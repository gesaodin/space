  ...
  */
  function GarantiasDiasAdicionales($archivo =  '',  $tipo = 0, $porce = 100){
    ...
    if($tipo == 2){
      ...
      $comando = 'cd tmp/; awk -F\';\' \'{ for (x=1; x<=34; x++) {  printf "%s;", $d } printf $' .
      '' . $porcen . ' "\n" } \' ' . $archivo . '.csv >> ' . $file . '/' . $file . '.csv';
      exec($comando, $firma);
    }else if($tipo == 1){
      //**SE MODIFICO INCLUYENDO CONDICION PARA QUE MONTO DA<0 NO SALGA EN EL REPORTE FINAL
      $comando = 'cd tmp/; awk -F\';\' \' $31 > 0 { for (x=1; x<=30; x++) {  printf "%s;", $d } printf $' .
      $porcen . ' "\n" } \' ' . $archivo . '.csv >> ' . $file . '/' . $file . '.csv';
      exec($comando, $firma);
     }else if($tipo == 0){
      $comando = 'cd tmp/; awk -F\';\' \' { for (x=1; x<=30; x++) {  printf "%s;", $d } printf $' .
      $porcen . ' "\n" } \' ' . $archivo . '.csv >> ' . $file . '/' . $file . '.csv';
      exec($comando, $firma);
     }
